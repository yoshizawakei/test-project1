    ...
    <form action="/items" method="post" enctype="multipart/form-data" class="product-form">
        ...
        <section class="form-section">
            ...
            <div class="form-item">
                ...
                <div class="category-buttons" id="category-buttons-container">
                    @foreach ($categories as $category)
                        <button type="button" class="category-button {{ in_array($category->id, old('category_ids', [])) ? 'selected' : '' }}" data-category-id="{{ $category->id }}">
                            {{ $category->name }}
                        </button>
                        <input type="hidden" name="category_ids[]" value="{{ $category->id }}" class="category-hidden-input" data-category-id="{{ $category->id }}" {{ in_array($category->id, old('category_ids', [])) ? '' : 'disabled' }}>
                    @endforeach
                </div>
                @error("category_ids")
                    <div class="error-message">{{ $message }}</div>
            ...
            <div class="form-item">
                ...
                <div class="select-wrapper">
                    <select id="product-status" name="condition" class="form-control @error("condition") is-invalid @enderror">
        ...
        <section class="form-section">
            ...
            <div class="form-item">
                ...
                <input type="text" id="product-name" name="item_name" value="{{ old("item_name") }}" placeholder="商品名を入力してください" required class="form-control @error('item_name') is-invalid @enderror">
                @error("item_name")
                    <div class="error-message">{{ $message }}</div>
                @enderror
            </div>
            <div class="form-item">
                ...
                <div class="select-wrapper">
                    <select id="brand-name" name="brand_id" class="form-control @error('brand_id') is-invalid @enderror">
            ...
            <div class="form-item">
                ...
                <textarea id="product-description" name="description" rows="5" placeholder="商品の説明を入力してください" required class="form-control @error("description") is-invalid
            ...
            <div class="form-item">
                ...
                <div class="form-item price-input">
                    <input type="number" id="selling-price" name="price" value="{{ old("price") }}" placeholder="¥" min="0" required class="form-control @error('price') is-invalid @enderror">
                </div>
                @error("price")
                    <div class="error-message">{{ $message }}</div>
                @enderror
